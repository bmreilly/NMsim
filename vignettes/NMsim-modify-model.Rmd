...
The following compares the `$THETA`, `$OMEGA`, and `$SIGMA` sections of the estimated and the simulated control streams side-by-side. The values have been updated in the simulation control stream based on the `.ext` file.
...
my.file.diff <- function(file1,file2,sections=NULL){
    ...
    waldo::compare(res.file1,res.file2,ignore_attr=TRUE)
}
## diffr::diffr(file.mod,modTab(simres)$path.sim)

secs <- c("THETA","OMEGA","sigma")
my.file.diff(file.mod,modTab(simres)$path.sim,sections=secs)

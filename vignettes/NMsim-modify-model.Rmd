...
```{r,echo=FALSE}
my.file.diff <- function(file1,file2){
    waldo::compare(readLines(file1),readLines(file2),ignore_attr=TRUE)
}
## diffr::diffr(file.mod,modTab(simres)$path.sim)
my.file.diff(file.mod,modTab(simres)$path.sim)
```
...
my.file.diff(file.mod,modTab(simres.inits1)$path.sim)
```

...
```{r}
mods <- modTab(simres)[,.(file.mod,path.sim)] |>
    melt(measure.vars=c("file.mod","path.sim"))
##ctls <- setNames(lapply(mods$value,as.NMctl),cc(file1,file2))
diffr::diffr(mods$value[1],mods$value[2])
## mods[,
##      as.NMctl(value)
##      ] |> diffr::diffr()
...
simres.inits1 <- NMsim(file.mod=file.mod,
                       ...
                       name.sim="inits1")
modTab(simres.inits1)[,diffr::diffr(file1=file.mod,file2=path.sim)]
```

...
simres <- NMsim(file.mod=system.file("examples/nonmem/xgxr021.mod",package="NMsim"),
                data=data.sim,
                dir.sims="vignettes/simulate-results")
```

```{r,include=FALSE,eval=TRUE}
simres <- NMreadSim("vignettes/simulate-results/xgxr021_noname_MetaData.rds")
```
...
This example was a simulation of a multiple dose regimen with a loading dose using a model estimated on single dose data. It is from the first vignette [`NMsim-intro.html`](https://NMautoverse.github.io/NMsim/articles/NMsim-intro.html). Go there next to get started with `NMsim`.
